...
-   [Temporal Disaggregation](#a-bench-tempd): in [GUI](#a-bench-tempd-gui) and [R](#a-bench-tempd-r) ,
...
are Regression model and Model-based Denton. Temporal disaggregation methods available in R include Regression models (Temporal disaggregation, Interpolation, Reverse regression, Autoregressive distributed lag (ADL)) and Model-based Denton method.
...
2.  **Shocks**: A list specifying the shocks and their magnitude (variance). For each shock,     specify the Position ("YYYY-MM-DD") and the Variance (with 1 indicating the normal situation).
