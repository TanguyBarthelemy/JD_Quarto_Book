# Spectral Analysis Tools {#m-spectrum .unnumbered}

## In this chapter

This chapter provides some guidance in order to understand the spectral analysis tools available in JDemetra+ [Graphical User Interface](#t-GUI-tstools-spectral).

<!-- ADD  -->
<!-- and in R ? -->

<!-- List of tools, links and tests -->

<!-- and also to understand some seasonality tests ? -->

<!-- and [R packages](#t-r-packs) -->
<!-- what do we have in R ? toolkit or SA packs: nothing, rjd3filter!: not really spectral approach -->
...
## Spectral density of a stationnary process

A stationary time series ${x_t: t = 1, \dots T}$ with mean $μ$ and autocovariance $\gamma(k)=E((x_{t}-\mu)(x_{t-k}-\mu))=$ can be described as a weighted sum of periodic trigonometric functions: $sin(\omega t)$ and $cos(\omega t)$, where $\omega=\frac{2*pi*k}{T}, k: 1 \dots [\frac{T}{2}]$ denotes the (angular) Fourier frequencies. 

<!-- ADD : units  -->
<!-- equation -->

Spectral analysis investigates this frequency domain representation of $x_{t}$ to determine how important components of different frequencies are in accounting for the behaviour of $x_{t}$.

Assuming that the autocovariances $\gamma(k)$ are absolutely summable($\sum_{k=-\infty}^{\infty}|\gamma(k)|<\infty$), the spectral density of the process is defined by (HAMILTON, J.D. (1994))

$$
f(\omega)=\frac{1}{2\pi}\sum_{k=-\infty}^{\infty}{e^{-ik\omega}\gamma(k)}
$$ {#eq-1}

where $e^{-ik\omega}=cos(k\omega)-isin(k\omega)$, $i=\sqrt{-1}$ and $-\infty < \ \omega < \infty$.

The spectral density is also called the population spectrum. 
The analysis of the population spectrum in the frequency domain is equivalent to the examination of the autocovariance function in the time domain, as by Fourier inversion, we can recover the autocovariances from the spectral density:

$$
 \gamma (k) = \frac{1}{2 \pi} \int_{-\pi}^{\pi} f(\omega) e^{i\omega k} \, d\omega.
$$ {#eq-2}

- for $k=0$, we see the process' variance is the average integral of $f$.

- $f(\omega)\text{dω}$ is interpreted as a contribution to the variance of components with frequencies in the range $(\omega,\ \omega+d\omega)$, a peak in the spectrum indicates an important contribution to the variance at the corresponding frequencies

Spectral density can also be expressed

$$
f(\omega)=\frac{1}{2\pi}\sum_{k=-\infty}^{\infty}{(cos(\omega k)-isin(\omega k)\gamma(k)}
$$ {#eq-3}

or 

$$
f(\omega)=\frac{1}{2\pi}[\ \gamma(0)+2\sum_{k=1}^{\infty}{\ \gamma(k)}cos(\omega k)]
$$ {#eq-4}

This implies that if autocovariances are absolutely summable then the population spectrum exists and is a continuous, real-valued function of $\omega$. 

Due to the properties of trigonometric functions $(\cos(-\omega k)=\cos(\omega k)$ and $\cos(\omega+2\pi j)k=cos(\omega k))$ the spectrum is a periodic, even function of $\omega$, symmetric around $\omega=0$. Therefore, the analysis of the spectrum can be reduced to the interval $(-\pi,\pi)$. The spectrum is non-negative for all $\omega \in (-\pi,\pi)$.

The fastest oscillation that can be observed in a time series sampled at discrete time intervals ($\Delta t$) lasts two sampling periods ($2\Delta t$), it corresponds to the fastest observable  frequency : $\omega=\frac{2\pi}{2\Delta t}=\frac{\pi}{\Delta t}$ called the Nyquist frequency. The slowest oscillation that can be observed in the time series with $n$ observations corresponds to the lowest frequency $\omega=\frac{2\pi}{n}$ and is called the fundamental (Fourier) frequency.

Note that if $x_{t}$ is a white noise process with zero mean and variance $\sigma^{2}$, then for all $|k|> 0$ $\gamma(k)=0$, $\gamma(0)=\sigma^{2}$. Hence the spectrum of $x_{t}$ is constant ($f(\omega)=\frac{\sigma^{2}}{2\pi}$) showing that each frequency contributes equally to the variance of the process (BROCKWELL, P.J., and DAVIS, R.A. (2002)).

The aim of spectral analysis is to determine how important components of different frequencies are in accounting for the behaviour of a time series. Since spectral analysis can be used to detect the presence of periodic components, it is a natural diagnostic tool for detecting seasonality as well as trading day effects.

In practice the spectral density (population spectrum) has to be estimated. In JDemetra+, it is done in two ways: 

    - using the sample spectrum also called periodogramm
    
    - using an ARIMA representation of the process

The explanations given in the remainder of this chapter derive mainly from DE ANTONIO, D., and PALATE, J. (2015) and BROCKWELL, P.J., and DAVIS, R.A. (2006).

## Estimation of the Spectral density

### Method 1: Periodogram {#m-spectral-analysis-m-spectrum-periodogram}

For any given frequency $\omega$ the sample periodogram is the sample analog of the population spectrum. It will be used as an estimator of the spectral density, with certain drawbacks.

<!-- REDEF -->
<!-- In general, the periodogram is used to identify the periodic components of unknown frequency in the time series. X-13ARIMA-Seats and Tramo-Seats use this tool for detecting seasonality in raw time series and seasonally adjusted series. Apart from this it is applied for checking randomness of the residuals from the ARIMA model. -->
